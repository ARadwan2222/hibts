...
class AchievementsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val (level, levelName) = AchievementEngine.getLevel(requireContext(), totalXP)
        tvXP?.text   = getString(R.string.total_xp).format(totalXP)
        tvLevel?.text = getString(R.string.level_display).format(level, levelName)
        ...
        db.achievementDao().getAllAchievements().observe(viewLifecycleOwner) { unlocked ->
            ...
            val allDefs = AchievementDefinitions.getAll(requireContext())
